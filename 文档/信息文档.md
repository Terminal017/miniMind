...
1、pnpm：流行的前端包管理器，用于解决 npm 包管理的缺陷。

2、npm 包管理器问题：采用扁平化的存储方式，存在依赖重复安装，磁盘存储浪费和幽灵依赖的问题。

3、pnpm 原理：采用全局存储 + 硬链接 + 符号链接的方式

1. 执行安装命令后，所有包只会被下载一次，并保存到全局的中`.pnpm-store`。它依赖与文件的哈希值存储，保证相同的文件只会存储一次。
2. 然后 pmpm 会**在`.pnpm`下的包创建指向全局存储中文件的硬链接**（指向 `.pnpm-store` 里的物理数据）。它们是系统级引用，不占用额外空间。
3. pnpm 的 `node_modules` 实际上是一个复杂的 **符号链接** 网，所有的包均以`pkg@version` 的形式平铺在`.pnpm` 目录（虚拟存储），项目根目录中**只会**出现你在 `package.json` 里写的包，它们其实是指向 `.pnpm/` 目录下对应包的**符号链接**。

### 十四、其他相关库

1、react-dropzone：一个简单的 React Hook，用于创建符合 HTML5 标准的文件拖放区域。
